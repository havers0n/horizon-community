// Тестовый скрипт для проверки RPC функции напрямую в Supabase
...
async function testSupabaseDepartments() {
  ...
  try {
    // Тест 3: Проверяем схему public
    console.log('\n3️⃣ Тестируем таблицу departments в схеме public...');
    const { data: publicData, error: publicError } = await supabase
      .from('departments')
      .select('*');
    
    if (publicError) {
      console.error('❌ Ошибка запроса public.departments:', publicError);
    } else {
      console.log('✅ Запрос public.departments выполнен');
      console.log('Количество департаментов в public:', publicData?.length || 0);
    }
    
  } catch (error) {
    console.error('❌ Исключение при запросе public.departments:', error);
  }
}
