...
export type Purpose = 'ENTRY' | 'PROMOTION' | 'QUALIFICATION'
...
export interface UpdateTestDto extends Partial<Omit<CreateTestDto, 'purpose' | 'target'>> {
	target?:
		| { department_id: string }
		| { rank_id: string }
		| { qualification_id: string }
...
// Dictionaries
export interface DepartmentDto { id: string; name: string }
