...
export interface WebSocketClient {
  ws: any; // WebSocket; // Changed to any for now to avoid circular dependency
...
class CADWebSocketServer {
  ...
  constructor(server: any) { // Changed to any for now to avoid circular dependency
  ...
  private setupWebSocket() {
    this.wss.on('connection', (ws: any, request) => { // Changed to any for now to avoid circular dependency
      const clientId = this.generateClientId();
      const client: WebSocketClient = {
        ws: ws as any, // Changed to any for now to avoid circular dependency
        permissions: [],
        subscriptions: []
      };
  ...
  private sendToClient(clientId: string, event: WebSocketEvent) {
    const client = this.clients.get(clientId);
    if (!client || client.ws.readyState !== 1) return; // Changed to 1 for readyState
  ...
  private startHeartbeat() {
    this.heartbeatInterval = setInterval(() => {
      this.clients.forEach((client, clientId) => {
        if (client.ws.readyState === 1) { // Changed to 1 for readyState
          this.sendToClient(clientId, {
...
export function initializeCADWebSocket(server: any): CADWebSocketServer { // Changed to any for now to avoid circular dependency
