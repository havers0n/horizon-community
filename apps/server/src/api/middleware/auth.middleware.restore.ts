...
export async function authenticateToken(
	...
	try {
		console.log('[AuthMiddleware] authenticateToken started.');
		...
		console.log('[AuthMiddleware] User profile fetched:', profile);
		...
		try {
			console.log('[AuthMiddleware] Building session for user:', authenticatedUser.id);
			const supa = (req as any).supabase;
			if (!supa) {
				console.error('[AuthMiddleware] Missing per-request Supabase client');
				res.status(500).json({ success: false, error: 'Server configuration error: missing per-request Supabase client' });
				return;
			}

			const userId = authenticatedUser.id;
			console.log('[AuthMiddleware] User ID:', userId);
			...
			if (charactersRes.error) {
				console.error('[AuthMiddleware] Error fetching characters:', charactersRes.error);
			...
			console.log('[AuthMiddleware] User roles fetched:', rolesRes.data);
			...
			console.log('[AuthMiddleware] Final session object attached to request:', req.session);
...
export async function requireCharacter(
  ...
  try {
    ...
    // Если есть несколько персонажей, проверяем заголовок X-Character-ID
    const characterId = req.headers['x-character-id'] as string;
    if (!characterId) {
      res.status(400).json({
        success: false,
        error: 'Multiple characters found. Please specify character ID with X-Character-ID header',
