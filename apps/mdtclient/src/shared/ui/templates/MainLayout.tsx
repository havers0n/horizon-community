...
const Header: React.FC<HeaderProps> = ({ currentRole, onRoleChange, availableRoles, user, onLogout }) => {
    ...
    if (isUnitRole) {
        if (currentRole === UserRole.LEO) {
            displayUnit = MOCK_UNITS.find(u => u.departmentId === 1) || MOCK_UNITS[0];
            displayCitizen = MOCK_CITIZENS[0];
        } else if (currentRole === UserRole.EMS_FD) {
            displayUnit = MOCK_UNITS.find(u => u.departmentId === 3) || MOCK_UNITS[2];
        ...
        <header className="bg-slate-900/80 backdrop-blur-md border-b border-secondary-700/50 p-3 flex justify-between items-center sticky top-0 z-30">
                 ...
                 {isUnitRole && displayCitizen && displayUnit && (
                     <div className="hidden md:flex items-center gap-3 border-l-2 border-secondary-700/50 pl-4">
                        <div>
                            ...
                            <p className="text-xs text-secondary-400 leading-tight">{displayUnit.departmentId === 1 ? 'LSPD' : displayUnit.departmentId === 2 ? 'BCSO' : 'LSFD'}</p>
                        </div>
                    </div>
                )}
            ...
            <div className="flex items-center gap-4 ml-4 flex-shrink-0">
                ...
                <div className="flex items-center gap-3 text-right">
                    {displayUnit && <span className="font-bold text-sm text-secondary-200">{displayUnit.unitNumber}</span>}
                    {user && (
