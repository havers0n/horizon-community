...
export const ThemeWrapper: React.FC<ThemeWrapperProps> = ({
  ...
  const { currentPreset, applyThemeToElement } = useTheme();
    ...
    if (wrapperRef.current) {
      ...
      childElements.forEach((element) => {
        if (element instanceof HTMLElement) {
          const childLayer = element.getAttribute('data-theme-layer') as
            | 'card'
            | 'button'
            | 'input';
  ...
  return (
    <div ref={wrapperRef} className={className} data-theme-layer={layer}>
      {children}
    </div>
  );
};
